...
public class RobotContainer {
  ...
  public RobotContainer() {
    ...
    autos.addOption("2 Piece", Autos.twoPiece(drive, intake, shooter));
  ...
  private void configureBindings() {
    this.controller.manualIntake().onTrue(intake.intake(shooter));

    this.controller.scoreHigh().onTrue(shooter.scoreProtected());

    this.controller.adjustUp().whileTrue(shooter.adjust(true, intake));
    this.controller.stuck().onTrue(shooter.stuck());
    ...
    this.controller.intake().whileTrue(new AutoIntake(drive, intake, shooter));
